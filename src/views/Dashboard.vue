  ...
  <div class="grid">
    <div class="col-12">
      <div class="card">
        ...
        <div
          v-if="MEMBER.LIST?.list?.length > 0"
          class="p-paginator p-component mt-2"
        >
          <button
          ...
          <span class="p-paginator-pages" data-pc-section="pages">
            <!-- pageNow -->
            <button
              v-for="(v, i) in MEMBER.LIST?.page"
              :key="i"
              :class="{ 'p-highlight': MEMBER.LIST?.pageNow === i + 1 }"
              class="p-paginator-page p-paginator-element p-link"
              type="button"
              @click="onClickPageNation(i + 1)"
            >
              {{ i + 1 }}
            </button>
          </span>
          <button
            ...
            type="button"
            :class="{
              'p-disabled': MEMBER.LIST?.pageNow === MEMBER.LIST?.page,
            }"
            :disabled="
              MEMBER.LIST?.pageNow === MEMBER.LIST?.page ? true : false
            "
            @click="onClickPageNation(MEMBER.LIST?.pageNow + 1)"
          >
...
export default {
  ...
  methods: {
    ...
    onSubmitSearch() {
      ...
      this.ACTION_MEMBER_LIST(params);
      this.router.push(
        `/user/list?t=${this.dropdownValue.code}&q=${this.searchStr}`
      );
